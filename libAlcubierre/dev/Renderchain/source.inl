...
int VulkanRenderchainComponent::CreateGraphicsPipelines() {
    ...
    std::vector<IShaders::AlcShader*> shaders = {SL->fetchShader(0)};
    pipeline_geometry.init(parent->device->Device, parent->swapchain->ChainFormat, shaders);
...
VulkanRenderchainComponent::graphicsPipeline::~graphicsPipeline() {
    if(device != VK_NULL_HANDLE) {
        ...
        for(int i = 0; i < shaderModules.size(); i++) if(shaderModules[i]) vkDestroyShaderModule(device, shaderModules[i], nullptr);
...
int VulkanRenderchainComponent::opaqueGeometryPipe::init(VkDevice& _device, VkFormat& format, std::vector<IShaders::AlcShader*>& shaders) {
    device = _device;
    ...
    //Shaders
        ...
        std::vector<VkPipelineShaderStageCreateInfo> stageCreateInfos(shaders.size());
        std::vector<std::string> stageNames(shaders.size());

        //Each shader has 1 or more descriptor sets
        ...
        for(int i = 0; i < shaders.size(); i++) {
            ...
            VkShaderModule& shaderModule = shaderModules.emplace_back();
            vkCreateShaderModule(device, &shaderCreateInfo, nullptr, &shaderModule);

            //Reflect its properties
            SpvReflectShaderModule reflect;
            spvReflectCreateShaderModule(shaders[i]->size(), shaders[i]->data(), &reflect);

            stageNames[i] = reflect.entry_point_name;

            stageCreateInfos[i] = {
                .sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
                .stage = static_cast<VkShaderStageFlagBits>(reflect.shader_stage),
                .module = shaderModule,
                .pName  = stageNames[i].c_str()
            };

            for(int j = 0; j < reflect.descriptor_set_count; j++) {
                SpvReflectDescriptorSet& reflectedDescriptorSet = reflect.descriptor_sets[j];
                std::vector<VkDescriptorSetLayoutBinding> descriptorSetLayoutBindings(reflectedDescriptorSet.binding_count);

                for(int k = 0; k < reflectedDescriptorSet.binding_count; k++) {
                    SpvReflectDescriptorBinding* reflectedBinding = reflectedDescriptorSet.bindings[k];

                    descriptorSetLayoutBindings[k] = {
                ...
                VkDescriptorSetLayout& layoutInstance = descriptorSetLayouts.emplace_back();
                ...
                vkCreateDescriptorSetLayout(device, &createInfo, nullptr, &descriptorSetLayouts[i]);
            }

            spvReflectDestroyShaderModule(&reflect);
        }

        pipeCreateInfo.stageCount = stageCreateInfos.size();
        pipeCreateInfo.pStages = stageCreateInfos.data();
...
int VulkanRenderchainComponent::createObjectBuffer(int*& modelBufferIndex, IGraphicsBackend::modelData& data) {
    //temp
        IGraphicsBackend::modelData tempdata;
        tempdata.vertices = {Vector(0.0f, 0.0f, 0.0f), Vector(0.5f, -1.0f, 0.0f), Vector(1.0f, 0.0f, 0.0f), Vector(1.5f, -1.0f, 0.0f)};
        tempdata.indices = {0, 1, 2, 1, 2, 3};
